...
Leave the file in a conflicted state for you to resolve after your update
is complete.
\item[\texttt{(tf)\ theirs-full\ {[}theirs-full{]}}]
Discard all of your local changes to the file and use the incoming
version of the entire file from the repository.
\item[\texttt{(mf)\ mine-full\ {[}mine-full{]}}]
Reject all of the incoming changes for the file and keep your local
version of the entire file.
\item[\texttt{(tc)\ theirs-conflict\ {[}theirs-conflict{]}}]
Accept the incoming changes from the repository only in the regions where
they conflict with your local changes; keep your local changes
everywhere else.
\item[\texttt{(mc)\ mine-conflict\ {[}mine-conflict{]}}]
Reject the incoming changes from the repository in the regions where they
conflict with your local changes, but accept all of the non-conflicting
incoming changes.
\item[\texttt{(r)\ resolved\ {[}working{]}}]
Accept the file as it currently appears in your working copy. Use this
once you have manually resolved the conflict---for example, after editing
the file---to tell Subversion that the conflict markers are gone and the
file is ready.
\item[\texttt{(q)\ quit}]
Postpone this and all remaining conflicts, leaving them to be resolved
...
Display, in unified diff format, all of the changes that have been merged
into the conflicted file.
\item[\texttt{(dc)\ display-conflict}]
Display only the conflicting regions of the file, ignoring changes which
were merged successfully.
\item[\texttt{(m)\ merge}]
Resolve the conflict interactively with a merge tool---by default
Subversion\textquotesingle s built-in tool, or an external tool if you
have configured one.
\item[\texttt{(l)\ launch\ {[}launch{]}}]
Launch an external program to perform the conflict resolution. This
requires a bit of preparation beforehand.
\item[\texttt{(i)\ internal}]
Resolve the conflict using Subversion\textquotesingle s built-in
interactive merge tool. This option is available starting with Subversion
1.8.
...
This shows, in unified diff format, how your conflicted working file
(\texttt{MERGED}) differs from the incoming repository version
(\texttt{THEIRS}). Within the conflicting region, the markers delimit
your local change (the \texttt{.mine} section), the original
common-ancestor text (the \texttt{|||||||} section, added in Subversion
1.9), and the incoming change (the \texttt{.r2} section).
...
The strings of less-than signs, vertical bars, equals signs, and
greater-than signs are conflict markers and are not part of the actual
data in conflict. You generally want to ensure that those are removed
from the file before your next commit. Subversion divides the conflicting
region into three parts. The text between the first two markers is
...
The text between the \texttt{|||||||} and \texttt{=======} markers shows
the original text from the common ancestor revision (added in Subversion
1.9). Here it is empty, because both you and Sally inserted new lines
where the base file had none:
...
And the text between the last two conflict markers is the text from
Sally\textquotesingle s commit:
