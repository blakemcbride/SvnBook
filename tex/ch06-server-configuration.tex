...
should always use \texttt{{[}path{]}} form only. Authenticated usernames
...
repository is wholly owned by the \texttt{svn} group, these newly
created log files won\textquotesingle t necessarily be owned by that
same group, which then creates more permissions problems for your users.
A good workaround is to set the group SUID bit on the
