...
\item[\protect\phantomsection\label{svn.ref.svn.sw.adds_as_modification}{}\texttt{-\/-adds-as-modification}]
When updating, if an incoming addition lands on a path that you have also
scheduled for addition, merge the two instead of raising a tree conflict.
(Subversion 1.14.)
...
\item[\protect\phantomsection\label{svn.ref.svn.sw.human_readable}{}\texttt{-\/-human-readable}
(\texttt{-H})]
Show file sizes with base-2 unit suffixes (for example, \texttt{1.7K} or
\texttt{4.0M}) rather than as exact byte counts. Used by \texttt{svn\ info}
and, in verbose mode, by \texttt{svn\ list}. (Subversion 1.12.)
...
\item[\protect\phantomsection\label{svn.ref.svn.sw.include_externals}{}\texttt{-\/-include-externals}]
Also operate on externals defined by \texttt{svn:externals} properties,
for the subcommands that support it (such as \texttt{svn\ status},
\texttt{svn\ info}, \texttt{svn\ list}, \texttt{svn\ commit},
\texttt{svn\ copy}, and \texttt{svn\ cleanup}). (Subversion 1.9 and
later.)
...
\item[\protect\phantomsection\label{svn.ref.svn.sw.no_newline}{}\texttt{-\/-no-newline}]
Do not print the customary trailing newline at the end of the
command\textquotesingle s output. Useful with \texttt{svn\ propget} and
\texttt{svn\ info\ -\/-show-item} when capturing a single value in a
script.
...
\item[\protect\phantomsection\label{svn.ref.svn.sw.pin_externals}{}\texttt{-\/-pin-externals}]
When copying, pin any externals that lack an explicit revision to the
revision they currently resolve to, recording that revision in the new
copy\textquotesingle s \texttt{svn:externals} property. (Subversion 1.9.)
...
changelist.
\item[\protect\phantomsection\label{svn.ref.svn.sw.remove_added}{}\texttt{-\/-remove-added}]
When used with \texttt{svn\ revert}, reverting a scheduled addition also
removes the item from disk instead of leaving it behind as an unversioned
file. (Subversion 1.14.)
\item[\protect\phantomsection\label{svn.ref.svn.sw.remove_ignored}{}\texttt{-\/-remove-ignored}]
When used with \texttt{svn\ cleanup}, remove ignored files and
directories from the working copy. (Subversion 1.9.)
\item[\protect\phantomsection\label{svn.ref.svn.sw.remove_unversioned}{}\texttt{-\/-remove-unversioned}]
When used with \texttt{svn\ cleanup}, remove unversioned files and
directories from the working copy. (Subversion 1.9.)
...
\item[\protect\phantomsection\label{svn.ref.svn.sw.show_item}{}\texttt{-\/-show-item}
\textless ARG\textgreater{}]
When used with \texttt{svn\ info}, print only the value of the single
requested field---for example, \texttt{revision}, \texttt{url},
\texttt{relative-url}, \texttt{repos-root-url}, \texttt{repos-uuid},
\texttt{last-changed-revision}, or \texttt{wc-root}---which is convenient
for scripting. (Subversion 1.9.)
...
This option is accepted by all \texttt{svn} subcommands.
\item[\protect\phantomsection\label{svn.ref.svn.sw.vacuum_pristines}{}\texttt{-\/-vacuum-pristines}]
When used with \texttt{svn\ cleanup}, remove unreferenced files from the
working copy\textquotesingle s pristine store inside the \texttt{.svn}
directory. (Subversion 1.10.)
...
\section{svn auth}\label{svn.ref.svn.c.auth}

\emph{Manage cached authentication credentials.}

\textbf{Synopsis}

\texttt{svn\ auth\ {[}PATTERN...{]}}

\texttt{svn\ auth\ -\/-remove\ PATTERN\ {[}PATTERN...{]}}

\subsection{Description}

Subversion caches authentication credentials---usernames, passwords, SSL
certificates, and SSL client-certificate passphrases---on disk so that
you are not prompted for them on every operation. The \texttt{svn\ auth}
command lets you inspect and clean out that cache.

With no arguments, \texttt{svn\ auth} lists all cached credentials. If
one or more \texttt{PATTERN} arguments are given, only credentials whose
attributes match \emph{all} of the patterns are listed; patterns are
matched case-sensitively and may contain the glob wildcards \texttt{?}
(any single character), \texttt{*} (any sequence of characters), and
\texttt{{[}abc{]}} (any listed character). With the \texttt{-\/-remove}
option, the matching credentials are deleted from the cache rather than
listed.
...
Show only the credentials for a particular host, including the cached
passwords:
