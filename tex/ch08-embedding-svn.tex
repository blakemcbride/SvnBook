...
The legacy Berkeley DB filesystem backend
\item[libsvn\_fs\_fs]
...
through using your favorite shell program), it uses an abstract storage
backend---today, a flat-file representation known as FSFS. (To learn
more about the repository backend and its history, see
...
the transaction support provided by an underlying database. Both types of
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/ch08dia1.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/ch08dia2.png}}
...
mechanism for data passed through its API), and while a person coding
against the
...
As of Subversion 1.14, the SWIG Python bindings (and
Subversion\textquotesingle s test suite) support Python 3; support for
the older Python 2 is deprecated and may be dropped in a later 1.14.x
point release. Building the Python bindings now also requires the
\texttt{py3c} Python 3 compatibility library.
