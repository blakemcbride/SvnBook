...
\item[\protect\phantomsection\label{svn.ref.svnadmin.sw.ignore_dates}{}\texttt{-\/-ignore-dates}]
When loading a dump stream, ignore the revision datestamps it contains,
so the loaded revisions are stamped with the time of loading instead.
...
\item[\protect\phantomsection\label{svn.ref.svnadmin.sw.no_flush_to_disk}{}\texttt{-\/-no-flush-to-disk}]
Disable the usual flushing of data to disk during the operation. This
speeds up a bulk load at the cost of durability should the machine crash
partway through.
\item[\protect\phantomsection\label{svn.ref.svnadmin.sw.normalize_props}{}\texttt{-\/-normalize-props}]
When loading a dump stream, normalize the encoding and line endings of
property values that need it (such as \texttt{svn:*} properties) instead
of rejecting nonconforming values. (Subversion 1.10.)
