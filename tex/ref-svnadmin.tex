...
\begin{description}
\item[\protect\phantomsection\label{svn.ref.svnadmin.sw.bdb_log_keep}{}\texttt{-\/-bdb-log-keep}]
(Berkeley DB-specific.) Disable automatic log removal of database
logfiles. Having these logfiles around can be convenient if you need to
restore from a catastrophic repository failure.
\item[\protect\phantomsection\label{svn.ref.svnadmin.sw.bdb_txn_nosync}{}\texttt{-\/-bdb-txn-nosync}]
(Berkeley DB-specific.) Disables fsync when committing database
transactions. Used with the \texttt{svnadmin\ create} command to create
a Berkeley DB-backed repository with \texttt{DB\_TXN\_NOSYNC} enabled
(which improves speed but has some risks associated with it).
...
values.
\item[\protect\phantomsection\label{svn.ref.svnadmin.sw.clean_logs}{}\texttt{-\/-clean-logs}]
...
requested filesystem type. \textless ARG\textgreater{} may be either
\texttt{bdb} or \texttt{fsfs}.
...
handle. This is used for testing automatic repository recovery (a new
feature in Berkeley DB 4.4). It\textquotesingle s unlikely that
you\textquotesingle ll need to run this command.
...
but an FSFS repository can be created using the \texttt{-\/-fs-type}
option:
...
If you pass the \texttt{-\/-clean-logs} option, \texttt{svnadmin} will
perform a hot copy of your repository, and then remove unused Berkeley
DB logs from the original repository.
...
As described in
\hyperref[svn.berkeleydb.limitations.architectural]{Architectural
Limitations}, hot-copied Berkeley DB repositories are \emph{not}
portable across operating systems, nor will they work on machines with a
different ``endianness'' than the machine where they were created.

\section{svnadmin list-dblogs}\label{svn.ref.svnadmin.c.list-dblogs}

\emph{Ask Berkeley DB which logfiles exist for a given Subversion
repository (applies only to repositories using the \texttt{bdb}
backend).}

\textbf{Synopsis}

\texttt{svnadmin\ list-dblogs\ REPOS\_PATH}
...
\texttt{DB\_LOG\_AUTOREMOVE}, the logfiles accumulate, although most are
no longer used and can be deleted to reclaim disk space. See
\hyperref[svn.reposadmin.maint.diskspace]{Managing Disk Space} for more
information.

\section{svnadmin
list-unused-dblogs}\label{svn.ref.svnadmin.c.list-unused-dblogs}

\emph{Ask Berkeley DB which logfiles can be safely deleted (applies only
to repositories using the \texttt{bdb} backend).}

\textbf{Synopsis}

\texttt{svnadmin\ list-unused-dblogs\ REPOS\_PATH}
...
\emph{Bring a repository database back into a consistent state (applies
only to repositories using the \texttt{bdb} backend). In addition, if
