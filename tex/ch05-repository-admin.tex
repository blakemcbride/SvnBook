...
This book will continue to provide information relevant to
administrators of BDB-backed repositories where it makes sense to do so,
but most of this chapter will assume what the rest of the world does:
that FSFS is \emph{the} Subversion storage backend implementation.
Please refer to \hyperref[svn.berkeleydb]{The Berkeley DB Legacy
Filesystem} or to older versions of this documentation for more complete
information about administering such repositories.
...
(FSFS). You can explicitly choose the filesystem type using the
\texttt{-\/-fs-type} argument, which accepts as a parameter either
\texttt{fsfs} or \texttt{bdb}.
...
\texttt{svnadmin\ pack} has no effect on BDB-backed Subversion
repositories.
...
reasons for dumping and loading, including re-deploying a Berkeley DB
repository on a new OS or CPU architecture, switching between the
Berkeley DB and FSFS backends, or (as we\textquotesingle ll cover later
...
fact, the \texttt{cvs2svn} utility (see
\hyperref[svn.forcvs.convert]{Converting a Repository from CVS to
Subversion}) uses the dump format to represent the contents of a CVS
repository so that those contents can be copied into a Subversion
repository.
...
the entire repository directory (which includes either a Berkeley DB or
FSFS environment). Incremental backups are lesser things: backups of
...
exists for FSFS data. But you don\textquotesingle t have to implement
these algorithms yourself, because the Subversion development team has
already done so. The \texttt{svnadmin\ hotcopy} command takes care of
